...
import { getClassStringFromDescription } from "./get-classes";
shim();

export default function JsonToTS(json: any, userOptions?: Options): string[] {
  const { typeStructure, names } = getStructureAndNames(userOptions, json);

  return getInterfaceDescriptions(typeStructure, names).map(getInterfaceStringFromDescription);
}

function getStructureAndNames(userOptions: Options, json: any) {
  ...
  return { typeStructure, names };
}

export function JsonToClasses(json: any, userOptions?: Options): string[] {
  const { typeStructure, names } = getStructureAndNames(userOptions, json);

  return getInterfaceDescriptions(typeStructure, names).map(getClassStringFromDescription);
}

(<any>JsonToTS).default = JsonToTS;
module.exports = JsonToTS;

(<any>JsonToClasses).default = JsonToClasses;
module.exports = JsonToClasses;
